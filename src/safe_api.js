...
const ErrorMsg = CONSTANTS.ERROR_MSG;
...
export default class SafeApi {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        for (const publicName of publicNames) {
          let decryptedValue;
          try {
            decryptedValue = await window.safeMutableData.decrypt(publicNamesContainerHandle, publicName);
            const toString = String.fromCharCode.apply(null, new Uint8Array(decryptedValue));
            decryptedPublicNames.push(toString);
          } catch (err) {
            console.warn(`Failed to decrypt publicName ${err} - ${publicName}`);
          }
        }
  ...
  stringToCommentList(data) {
    const list = [];
    const jsonList = JSON.parse(data);
    for (let i = 0; i < jsonList.length; i++) {
      list.push(new CommentModel(jsonList[i].name, jsonList[i].message, jsonList[i].date));
    }
    return list;
  }

  updateEntry(commentModel, isDelete) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        do {
          ...
          try {
            ...
            updatedList = this.stringToCommentList(data.buf.toString());
            if (isDelete) {
            ...
            if (err.code === ErrorCode.VERSION_MISMATCH) {
              isVersionError = true;
            } else {
              throw err;
            }
          }
        } while (isVersionError);
        window.safeMutableDataMutation.free(mutationHandle);
        window.safeMutableDataEntries.free(entriesHandle);
        resolve(updatedList);
      } catch (err) {
  ...
  postComment(commentModel) {
    return new Promise(async (resolve, reject) => {
      try {
        let updatedList = this.comments.slice(0);
        ...
        if (updatedList.length === 0) {
          ...
          try {
            ...
            if (err.code === ErrorCode.ENTRY_EXISTS) {
              updatedList = await this.updateEntry(commentModel);
            } else {
              throw err;
            }
          }
  ...
  listComments() {
    return new Promise(async (resolve) => {
      try {
        const data = await window.safeMutableData.get(this.mData, this.topic);
        this.comments = this.stringToCommentList(data.buf.toString());
        resolve(this.comments);
      } catch (err) {
  ...
  deleteComment(commentModel) {
    return new Promise(async (resolve, reject) => {
      try {
        this.comments = await this.updateEntry(commentModel, true);
        resolve(this.comments);
      } catch (err) {
        reject(err);
      }
